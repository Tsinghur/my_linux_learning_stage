...
2. **epoll的边缘触发与水平触发模式的区别是什么？然后使用代码区分这两种模式的区别？**
   ...
   4. **为什么边缘触发一般配合recv的非阻塞模式**
   
      因为 ET 只通知一次，你必须**一次性把缓冲区数据读完**，直到读到 `EAGAIN / EWOULDBLOCK`
   
      如果用**阻塞 recv**：假设缓冲区有 100 字节，你只读了 50 字节就停了：
   
      - 剩下 50 字节还在缓冲区但 ET 已经触发过了，**不会再触发**
      - 阻塞 recv 会直接**卡死在这里**，等下一包数据
      - → 程序僵死，数据永远读不完
