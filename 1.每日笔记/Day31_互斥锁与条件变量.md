...
3. ==**条件判断时if会引起惊群效应（即如何解决虚假唤醒）**==

   - **惊群效应：一个信号叫醒了一堆等待的线程，但只有 1 个能干活，剩下的全白醒、白抢锁 —— 这就叫惊群效应。**

   - **对应你代码里的场景**
     你用了 `pthread_cond_broadcast`：

     - 队列空了 → **3 个消费者都在睡觉等待**
     - 生产者生产 1 个 → **广播叫醒 3 个消费者**
     - 结果：**只有 1 个能拿到数据**，另外 2 个醒来发现还是空，**只能继续回去睡觉**

     这**醒来又白等**的过程，就是**惊群效应**。

   - 为什么你必须用 `while` 不能用 `if`？
     ...
     - 用 `if`：被惊群叫醒后**不再检查**，直接去取数据 → **空指针、崩溃**
     - 用 `while`：被叫醒后**再检查一遍**，空就继续睡 → **安全**

     **惊群效应 = 一个信号叫醒一堆线程，但只有一个能干活。**

     **所以必须用 while 重新判断条件，防止出错。**