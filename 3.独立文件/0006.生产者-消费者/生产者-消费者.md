...
typedef struct queue_s { // 商品队列
    node_t *pHead; // 指向第一个商品
    node_t *pEnd; // 指向最后一个商品
    int size;
}queue_t;

void offer(queue_t *pQueue, int value) { // 提供一个价值为value的商品，将即加入容器中的商品队列
...
int fetch(queue_t *pQueue) // 拿走一个商品，返回其价值fetch_value，将其移出容器中的商品队列
{
    ...
    int fetchValue = pOldHead->value;
    free(pOldHead);
    return fetchValue;
}

void show(queue_t *pQueue) { // 把队列里的所有商品按顺序都打印出来，方便看队列里当前都有什么
